...
Extract k-th dispatch/available time per unit & build one vector of dispatch time, one vector of available times.
...
# 2️⃣ Function to assign closest station per call based on available units
build_scenario_ett <- function(data, scenario_name) {
  units <- scenario_units[[scenario_name]]
  
  # Keep track of units availability per station
  station_queue <- lapply(names(units), function(x) rep(0, units[[x]]))
  names(station_queue) <- names(units)
  
  data <- data %>%
    arrange(DT.DISP) %>%
    rowwise() %>%
    mutate(
      # Build vector of available stations
      available_stations = paste(names(units)[units > 0], collapse = ", "),
      
      # Get eTT values for available stations only
      eTT_values = list(c(FN = eTT.BG.FN, NN = eTT.BG.NN, Ce = eTT.BG.Ce, So = eTT.BG.So)[units > 0]),
      
      # Pick the station with minimum eTT
      closest_station = names(which.min(eTT_values)),
      travel_time_secs = min(eTT_values)
    ) %>%
    ungroup()
...
# 3️⃣ Generate datasets for each scenario
ems_S0 <- build_scenario_ett(ems, "S0")
ems_S1 <- build_scenario_ett(ems, "S1")
ems_S2 <- build_scenario_ett(ems, "S2")
ems_S3 <- build_scenario_ett(ems, "S3")
ems_S4 <- build_scenario_ett(ems, "S4")
...
### Convert to one dataset for LMM
...
# Mixed effects model: travel_time ~ scenario + (1 | assigned_station)
# Here, assigned_station is a random intercept
mod_lmm <- lmer(
  log_eff_time ~ scenario + (1 | assigned_station),
...
suppressPackageStartupMessages({
  # Only install if missing, no warnings printed
...
# Run contrasts quietly (suppress intermediate warnings about pbkrtest/lmerTest)
suppressWarnings({
  em <- emmeans(mod_lmm, ~ scenario)
  contrast(em, method = "trt.vs.ctrl", ref = "S0") %>%
