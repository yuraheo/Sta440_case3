...
build_scenario_units <- function(data, scenario_name) {
  ...
  for (i in 1:nrow(data)) {
    ...
    closest_station_vec[i] <- chosen_station
  ...
  data <- data %>%
    mutate(
      ...
      closest_station = closest_station_vec,
...
ems_long$scenario <- factor(ems_long$scenario)

# Mixed effects model: travel_time ~ scenario + (1 | closest_station)
# Here, closest_station is a random intercept
mod_mixed <- lmer(travel_time_secs ~ scenario + (1 | closest_station), data = ems_long)
...
```{r}
plot(fitted(mod_mixed), resid(mod_mixed),
     xlab = "Fitted values",
     ylab = "Residuals",
...
qqline(resid(mod_mixed), col = "red")
```
...
# ems_long <- ems_long %>%
#   group_by(call_id) %>%
#   mutate(travel_varies = travel_time_secs != first(travel_time_secs)) %>%
#   ungroup()
# mod_logit <- glm(travel_varies ~ REF.GRID + DISPATCH.PRIORITY.NAME + hour,
#                  data = ems_long,
#                  family = binomial)
# 
# ems_long <- ems_long %>%
#   mutate(prob_varies = predict(mod_logit, type = "response"))
# 
# # Confirm column exists
# head(ems_long$prob_varies)
# allocation_summary <- ems_long %>%
#   group_by(REF.GRID) %>%
#   summarize(
#     avg_pred_travel = mean(pred_travel, na.rm = TRUE),
#     avg_prob_varies = mean(prob_varies, na.rm = TRUE),
#     .groups = "drop"
#   ) %>%
#   arrange(desc(avg_prob_varies))
# 
# allocation_summary
