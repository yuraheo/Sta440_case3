...
We generated a dataset for each scenario using a queue-based simulation function. The simulation first sorted all calls by dispatch time, then iteratively assigned ambulances based on availability and proximity. For each station, the model tracked the next available time for every unit. When a call occurred, the algorithm selected the station with the shortest effective response time—combining travel and potential waiting time. If the nearest station’s units were all busy, the model automatically selected the next closest station with an available unit. After a unit was dispatched, its return time was updated according to the recorded duration between dispatch and availability.
...
The model diagnostics and goodness‑of‑fit results show that the mixed‑effects model fits the data reasonably well. The AIC (1286.5) and BIC (1318.8) values are both lower than in earlier model runs, which indicates that this version provides a better overall fit (Table 4). The marginal R² of about 0.05 means the station scenario itself explains around 5% of the variation in the log of response times, while the conditional R² of roughly 0.35 shows that the full model—including call‑specific random effects—accounts for about a third of the total variability (Table 4). The intraclass correlation coefficient (ICC ≈ 0.32) suggests that nearly one‑third of the variation in response time comes from consistent differences between calls, which supports keeping the random intercept for call_id in the model. Overall, these results suggest thatthe model effectively captures both the scenario-level effects and the impact of system load on EMS response performance.

The residuals‑versus‑fitted plot shows that the residuals are roughly centered around zero across the range of fitted values, with no strong pattern or funnel shape (Figure 4). This suggests that the model assumptions of linearity and constant variance are reasonable. The normal Q–Q plot indicates that the residuals follow an approximately normal distribution, as most points fall along the reference line with only minor deviations in the tails (Figure 5).
...
Overall, the analysis demonstrates that EMS station location and vehicle allocation have measurable effects on response performance across Vance County. Among the five scenarios, Scenario 3 with a near‑north station produced the most favorable results, reducing mean effective response times by roughly 13 percent compared with the current setup.The mixed‑effects model fit well according to diagnostic checks, and the scenario effects were statistically significant despite modest marginal R² values.
...
ggplot(ems_long_filt, aes(x = fitted, y = resid)) +
  ...
  labs(
    title = "Figure 6: Residuals vs Fitted Values by Scenario",
...
ggplot(emm_df, aes(x = scenario, y = response_min, fill = scenario)) +
  ...
  labs(
    title = "Figure 7: Predicted Response Times by Scenario (LMM)",
...
ggplot() +
  ...
  geom_point(data = emm_df,
             ...
             size = 2.5) +
  labs(
     title = "Figure 8: Predicted Response Times by Scenario with Observed Distributions",
