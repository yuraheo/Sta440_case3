...
While the LMM assumes normally distributed residuals, response time data is inherently positive and right skewed. So, to better reflect this distribution, we refit the model using a Gamma GLMM with a log link. Overall, this model preserves the non-negativity of response times and provides a more realistic representation of variability in emergency response performance. We also used a natural cubic spline for hour of the day to capture nonlinear patterns that are associated with traffic and call volume. 
...
First, we implemented our Linear Mixed Effects Model (LMM). We fit this model using the lmer() function with log transformed response time as the dependent variable. We used call ID as the random intercept to account for unobserved heterogeneity across calls. Then, we implemented our Gamma GLMM using the glmmTMB() function. Finally, we implemented a binomial logistic model using the glm() function with the family set to binomial.
...
emm_final <- emmeans(mod_final, ~ scenario, type = "response")
emm_df_final <- as.data.frame(emm_final)


ggplot(emm_df_final, aes(x = scenario, y = response, fill = scenario)) +
  geom_col(alpha = 0.7) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), width = 0.2) +
  labs(
    title = "Predicted Response Time by Scenario (Gamma GLMM)",
    y = "Predicted Mean Effective Response Time (seconds)",
    x = "Scenario"
  ) +
